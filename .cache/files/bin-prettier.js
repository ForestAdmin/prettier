...
var uncurryThis$c = bind$3 && bind$3.bind(call$6, call$6);
...
const readline = require$$0__default$1["default"];
...
function createLogger$1(logLevel = "log") {
  ...
  function createLogFunc(loggerName, color) {
    ...
    const prefix = color ? `[${chalk$1[color](loggerName)}] ` : "";
...
var getIterator$1 = function (argument, usingIterator) {
  ...
  if (aCallable$1(iteratorMethod)) return anObject$4(call$2(iteratorMethod, argument));
...
function normalizeDetailedOptionMap$1(detailedOptionMap) {
...
const constant$3 = constant$4;
const {
  normalizeDetailedOptionMap,
  createDetailedOptionMap
} = optionMap;

function getContextOptions$2(plugins, pluginSearchDirs) {
  const {
    options: supportOptions,
    languages
  } = prettier$6.getSupportInfo({
    showDeprecated: true,
    showUnreleased: true,
    showInternal: true,
    plugins,
  ...
  const detailedOptionMap = normalizeDetailedOptionMap(Object.assign(Object.assign({}, createDetailedOptionMap(supportOptions)), constant$3.options));
...
var getContextOptions_1 = getContextOptions$2;
...
function hasPath$1(object, path, hasFunc) {
  ...
  return !!length && isLength$2(length) && isIndex$1(key, length) && (isArray$9(object) || isArguments$2(object));
}
...
const camelCase$2 = (input, options) => {
...
camelcase.exports = camelCase$2; // TODO: Remove this for the next major release

camelcase.exports.default = camelCase$2;
...
var minimist$3 = function (args, opts) {
  ...
  function setArg(key, val, arg) {
    ...
    var value = !flags.strings[key] && isNumber$3(val) ? Number(val) : val;
  ...
  for (var i = 0; i < args.length; i++) {
    ...
    if (/^--.+=/.test(arg)) {
      ...
      if (!flags.unknownFn || flags.unknownFn(arg) !== false) {
        argv._.push(flags.strings['_'] || !isNumber$3(arg) ? arg : Number(arg));
      }
...
var arraySpeciesConstructor$1 = function (originalArray) {
  ...
  if (isArray$5(originalArray)) {
    ...
    if (isConstructor(C) && (C === Array$1 || isArray$5(C.prototype))) C = undefined;
...
const pick = pick_1;
const camelCase$1 = camelcase.exports;
const {
  optionsNormalizer: {
    normalizeCliOptions
  }
} = prettierInternal;
const getContextOptions$1 = getContextOptions_1;
const minimist$1 = minimist_1;
const createMinimistOptions$1 = createMinimistOptions$2;

function parseArgv$1(rawArguments, detailedOptions, logger, keys) {
  const minimistOptions = createMinimistOptions$1(detailedOptions);
  let argv = minimist$1(rawArguments, minimistOptions);
  ...
  return Object.fromEntries(Object.entries(normalized).map(([key, value]) => {
    const option = detailedOptions.find(({
      name
    }) => name === key) || {}; // If the flag is a prettier option, use the option name
    // `--plugin-search-dir` -> `pluginSearchDirs`
    // Otherwise use camel case for readability
    // `--ignore-unknown` -> `ignoreUnknown`

    return [option.forwardToApi || camelCase$1(key), value];
  }));
}

const detailedOptionsWithoutPlugins = getContextOptions$1().detailedOptions;

function parseArgvWithoutPlugins$2(rawArguments, logger, keys) {
  return parseArgv$1(rawArguments, detailedOptionsWithoutPlugins, logger, typeof keys === "string" ? [keys] : keys);
}

var parseCliArguments = {
  parseArgv: parseArgv$1,
  parseArgvWithoutPlugins: parseArgvWithoutPlugins$2
};
...
const getContextOptions = getContextOptions_1;
const {
  parseArgv,
...
const dashify = dashify$2; // eslint-disable-next-line no-restricted-modules
...
var getOptionsForFile_1 = getOptionsForFile$1;
...
const chalk = source; // eslint-disable-next-line no-restricted-modules
...
const getOptionsForFile = getOptionsForFile_1;
const isTTY = isTty;
...
async function formatStdin$1(context) {
  const filepath = context.argv.filepath ? path$1.resolve(process.cwd(), context.argv.filepath) : process.cwd();
  ...
  const relativeFilepath = context.argv.ignorePath ? path$1.relative(path$1.dirname(context.argv.ignorePath), filepath) : path$1.relative(process.cwd(), filepath);
...
async function formatFiles$1(context) {
  ...
  for await (const pathOrError of expandPatterns(context)) {
    ...
    const ignoreFilename = context.argv.ignorePath ? path$1.relative(path$1.dirname(context.argv.ignorePath), filename) : filename;
...
async function logFileInfoOrDie$1(context) {
  const {
    fileInfo: file,
    ignorePath,
    withNodeModules,
  ...
  const fileInfo = await prettier$3.getFileInfo(file, {
    ignorePath,
    withNodeModules,
    plugins,
    pluginSearchDirs,
  ...
  context.logger.log(prettier$3.format(stringify$1(fileInfo), {
    parser: "json"
  }));
}

var fileInfo = logFileInfoOrDie$1;

const path = require$$0__default$2["default"]; // eslint-disable-next-line no-restricted-modules

const prettier$2 = require$$3;

async function logResolvedConfigPathOrDie$1(context) {
...
var findConfigPath = logResolvedConfigPathOrDie$1;
...
const logFileInfoOrDie = fileInfo;
const logResolvedConfigPathOrDie = findConfigPath;
